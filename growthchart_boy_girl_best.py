...
age = st.sidebar.number_input("Enter Age (years):", min_value=5.0, max_value=18.0, step=0.5)
height = st.sidebar.number_input("Enter Height (cm):", min_value=60, max_value=200, step=1)
weight = st.sidebar.number_input("Enter Weight (kg):", min_value=15, max_value=140, step=1)
father_height = st.sidebar.number_input("Enter Father's Height (cm):", min_value=100, max_value=200, step=1)
mother_height = st.sidebar.number_input("Enter Mother's Height (cm):", min_value=100, max_value=200, step=1)
...
def calculate_sd_score(data, value):
    # Find the closest row for the given age
    differences = abs(data['Age'] - age)
    closest_idx = differences.idxmin()
    mean_value = data.iloc[closest_idx]['50']  # 50th percentile value
    sd = data.iloc[closest_idx]['SD']  # SD for that age
    return round((value - mean_value) / sd, 2)  # Return SD score rounded to 2 decimal places
...
def determine_centile_range(data, value):
    ...
    # Loop through the centile columns and determine the range
    ...
    # If below 3rd centile
    if value < row['3']:
        return "below the 3rd centile"

    # If above 97th centile
    if value > row['97']:
        return "above the 97th centile"


# Determine centile ranges for height and weight
...
# Display MPH
#st.subheader("Mid-Parental Height (MPH)")
#st.write(f"**Mid-Parental Height:** {mph} cm")


def calculate_age_for_value(data, value):
    # Find the closest age for the given value in the 50th percentile column
    differences = abs(data['50'] - value)
    closest_idx = differences.idxmin()
    closest_row = data.iloc[closest_idx]
    closest_age = closest_row['Age']

    return closest_age

# Correct height age and weight age calculation
if gender == "Boys":
    height_age = calculate_age_for_value(height_data, height)  # Use boys' height data
    weight_age = calculate_age_for_value(weight_data, weight)  # Use boys' weight data
else:
    height_age = calculate_age_for_value(height_data, height)  # Use girls' height data
    weight_age = calculate_age_for_value(weight_data, weight)  # Use girls' weight data

# Display the height age and weight age
...
# Function to calculate Target Height (TH)
def calculate_target_height(mph, gender):
    """
    Calculate Target Height (TH) using Mid-Parental Height (MPH) and the 50th percentile height for age 18.

    Parameters:
    mph (float): Mid-Parental Height
    gender (str): "Boys" or "Girls"

    Returns:
    float: Target Height (TH)
    """
    # Get the 50th percentile adult height for age 18 from the dataset
    if gender == "Boys":
...
# Calculate Target Height (TH)
target_height = calculate_target_height(mph, gender)

# Display MPH and Target Height
...
ax.set_xticks(range(5, 19, 1))  # Major ticks for every 1 year
ax.set_yticks(range(10, 201, 5))  # Major ticks for every 5 cm or 5 kg
ax.grid(True, which="major", linestyle="--", linewidth=0.5)
ax.legend(
    loc="upper left",            # Position the legend in the upper left
    fontsize=6,                  # Set a smaller font size
    title_fontsize=7,            # Set a slightly larger font for the legend title (if any)
    borderpad=0.5,               # Padding between the legend border and content
    labelspacing=0.5,            # Space between legend items
    handlelength=1.5,            # Length of the legend key handles
    handletextpad=0.5,           # Space between legend key and text
    borderaxespad=0.5,           # Padding between the legend and axes
    frameon=True,                # Add a border around the legend
    framealpha=0.9,              # Make the border slightly transparent
    ncol=1                       # Arrange legend items in one column
)

# Add the date of chart generation
date_generated = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
ax.text(17, 190, f"Generated on: {date_generated}", fontsize=8, ha="right", va="top")

# Display the height age and weight age
st.subheader("Calculated Age Equivalents")
#st.write(f"**Height Age:** {height_age} years")
#st.write(f"**Weight Age:** {weight_age} years")

# Display the chart
st.pyplot(fig)

# Option to download the chart
buf = BytesIO()  # Use BytesIO to handle binary data
fig.savefig(buf, format="png", dpi=300)  # Adjusted DPI for high resolution
buf.seek(0)  # Reset buffer to start

st.download_button(label="Download Growth Chart (A4 Size)", data=buf, file_name=f"{gender.lower()}_growth_chart_a4.png", mime="image/png")
